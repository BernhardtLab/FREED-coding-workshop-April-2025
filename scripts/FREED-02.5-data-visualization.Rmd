...
We will begin by once again loading in the tidyverse package that you previously installed.

The reason for including that here once again is the idea that in coding, an individual
R script/file should contain all of the commands required for it to be run, rather 
than depending on commands executed previously in other files. 
...
<input type="checkbox" id="solution-toggle-8" style="display:none;" />
 <label for="solution-toggle-8" class="toggle-label">Click to show the solution</label>
 <div id="solution-content-8" class="solution">
...
<input type="checkbox" id="solution-toggle-9" style="display:none;" />
 <label for="solution-toggle-9" class="toggle-label">Click to show the solution</label>
 <div id="solution-content-9" class="solution">
...
However, the result doesn't look quite as we might have expected: it seems to be jumping around a lot in each continent. It appears to be drawing one line per continent, but each continent contains many countries! 

Let's try to separate the data by country, plotting one line for each country:
...
Look carefully at the code above; the `color=continent` command is placed within the `geom_line()` command, not the `mapping` command.
...
<input type="checkbox" id="solution-toggle-10" style="display:none;" />
<label for="solution-toggle-10" class="toggle-label">Click to show the solution</label>
<div id="solution-content-10" class="solution">
...
<input type="checkbox" id="solution-toggle-11" style="display:none;" />
<label for="solution-toggle-11" class="toggle-label">Click to show the solution</label>
<div id="solution-content-11" class="solution">
<h4>Solution to challenge 4a:</h4> 
...
<input type="checkbox" id="solution-toggle-12" style="display:none;" />
<label for="solution-toggle-12" class="toggle-label">Click to show the solution</label>
<div id="solution-content-12" class="solution">
<h4>Solution to challenge 4b:</h4> 
...
Thus below the color legend title is set using `color = "Continent"`. We could
change this legend title from "Continent" to something else by adding the 
`fill = "Insert Title Here"` command to the `labs()` chunk.
...
<input type="checkbox" id="solution-toggle-13" style="display:none;" />
<label for="solution-toggle-13" class="toggle-label">Click to show the solution</label>
<div id="solution-content-13" class="solution">
<h4>Solution to challenge 5:</h4> 
...
::: bonus
# Bonus content: Reordering factor levels
:::

As you have seen in the plots above, countries are ordered alphabetically. However, there are reasons 
that we might conceivably want to change this arrangement. What if for example, we wanted the data for
countries from North America to be arranged at the top of this plot grid?

Here, we will convert country to a factor in the americas dataset and use `fct_relevel()` command to re-order them.

```{r}
americas$country.fac <- as.factor(americas$country)

levels(americas$country.fac)
```

The following countries are in South America: Argentina, Bolivia, Brazil, Chile, Colombia, Ecuador, Paraguay, Peru, Uruguay, and Venezuela.
We can also arrange Central American countries (Costa Rica, El Salvador, Guatemala, Honduras, Nicaragua, and Panama) in the middle.

We will use the `fct_relevel()` command from the forcats package (contained in the tidyverse) to do this

```{r}
americas$country.fac <- fct_relevel(americas$country.fac, "Canada", "Cuba", "Dominican Republic", "Haiti", "Jamaica", "Mexico",  "Puerto Rico", "Trinidad and Tobago", "United States","Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "Panama", "Argentina", "Bolivia", "Brazil", "Chile", "Colombia", "Ecuador", "Paraguay", "Peru", "Uruguay", "Venezuela")

levels(americas$country.fac)
```
Now we can specify that the `facet_wrap()` function should order things by this new division of countries into North and South America!

```{r}
ggplot(data = americas, mapping = aes(x = year, y = lifeExp, color=continent)) +
  geom_line() + facet_wrap( ~ country.fac) +
  labs(
    x = "Year",              # x axis title
    y = "Life expectancy",   # y axis title
    title = "Figure 1",      # main title of figure
