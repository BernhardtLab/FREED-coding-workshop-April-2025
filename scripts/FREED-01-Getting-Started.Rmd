...
### Project Directory Fundamentals 
...
'list.files' will list all the files in your current directory

```{r, eval=F}
list.files()
```

Moving around our directory requires the use of **file paths**. There are two way you can write file paths: 
1. Absolute path
2. Relative path

The absolute path defines every directory and subdirectory that a file is contained in, while a relative path only defines the directories relative to your working directory. Relative paths can be a lot quicker to write and they are not dependent on each individual user, making them more reproducible. For that reason, we will use relative file paths today (but see a challenge at the end if you want to test your absolute file path skills).
